...
public class StudentServiceImpl implements StudentService {
    ...
    public Page<StudentDTO> loadStudentsByName(String name, int page, int size) {
        ...
        Page<Student> studentsPage = studentDao.findStudentByName(name, pageRequest);
        return new PageImpl<>(studentsPage.getContent().stream().map(
                student -> studentMapper.fromStudent(student)).collect(Collectors.toList()), pageRequest, studentsPage.getTotalElements());
    }
